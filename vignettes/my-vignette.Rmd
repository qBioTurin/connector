...
The **GrowDataFile** is composed from two columns for each sample (i.e. PDX), containing the time points and the measure volume respectively. In our example, we have `r length(CONNECTORList$LabCurv[,1])` PDXs then the **GrowDataFile** is composed by `r length(CONNECTORList$LabCurv[,1])*2` columns.
