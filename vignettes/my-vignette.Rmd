...
Connector is built on the model-based approach for clustering functional data presented in [@james2003clustering], called Functional Clustering Model (FCM). Such method is particularly effective when the observations are sparse and irregularly spaced, as growth curves usually are. A review on the method and on the tools developed by the authors to suitably set the free parameters, please refer to [our NM paper].

[esempietto per capire che sono i dati!] 
...
IndexesPlot.Extrapolation(stability.list =ClusteringList )
