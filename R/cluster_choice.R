...
ClusterChoice<-function(data,k,h=1,p=5,PCAperc=NULL)
{
  ...
  for(k in K)
  {
    ...
    for(h in H)
    {
      ...
      within<-c()
      FCM.ClustCurve<-data.frame(ID=database[,1],Times=database[,3],Vol=database[,2],Cluster=rep(out.funcit@allClusters,times=data$LenCurv))
  ...
  ElbowMethod.Haus<-ggplot(data=Tot.within.Haus,aes(x=K))+ geom_point(aes(y=dist,col=H))+
    ...
    labs(title="Elbow method with Hausdorff distance",x="Cluster",y="total within-cluster")+
